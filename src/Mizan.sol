...
contract Mizan is Ownable {
    // TODO: Properly calculate LP returns.
    ...
    uint256 public profitSharePercentage = 10; // TODO: Make this configurable
    ...
    function setBnplContract(address _bnplContract) external onlyOwner {
        require(_bnplContract != address(0), "Invalid BNPL contract address");
        bnplContract = _bnplContract;
    }
    ...
    function requestBnplLoan(uint256 productId) external {
        ...
        // get product earnings from bnplContract
        uint256 productEarnings = BNPLCollateralLoan(bnplContract).getProductEarnings(productId);
        address productOwner = BNPLCollateralLoan(bnplContract).getProductOwner(productId);

        // Use only 50% of the liquidity for BNPL
        uint256 liquidityToUse = (totalLiquidity * 50) / 100;
